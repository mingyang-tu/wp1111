...
const useWordle = (solution) => {
    ...
    const handleEnter = () => {
        ...
        for (const item of solution.split('')) {
            if (Object.keys(solutionDict).includes(item)) {
                solutionDict[item]++;
            }
            else {
                solutionDict[item] = 1;
        ...
        for (let i = 0; i < curGuess.length; i++) {
            if (curGuess[i] === solution[i]) {
                newGuess.push({ char: curGuess[i], color: 'green' });
                solutionDict[solution[i]]--;
                count++;
                newUsedChars[solution[i]] = 'green';
            }
            else {
                newGuess.push({ char: curGuess[i] });
            }
        }
        for (let i = 0; i < newGuess.length; i++) {
            if (newGuess[i].color !== 'green') {
                if (Object.keys(solutionDict).includes(newGuess[i].char)) {
                    if (solutionDict[newGuess[i].char] > 0) {
                        newGuess[i].color = 'yellow';
                        solutionDict[newGuess[i].char]--;
                        if (newUsedChars[newGuess[i].char] !== 'green') {
                            newUsedChars[newGuess[i].char] = 'yellow';
                        }
                    }
                    else {
                        newGuess[i].color = 'grey';
                        if (newUsedChars[newGuess[i].char] !== 'green' && newUsedChars[newGuess[i].char] !== 'yellow') {
                            newUsedChars[newGuess[i].char] = 'grey';
                        }
                    }
                }
                else {
        ...
        setGuesses(
            [...guesses.slice(0, turn),
                newGuess,
            ...guesses.slice(turn + 1)]
        )
